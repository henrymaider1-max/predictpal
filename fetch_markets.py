...
def fetch_metaculus(limit=50):
    ...
    data = safe_get("https://www.metaculus.com/api2/questions/", params={
        "limit": limit, "order_by": "-activity", "status": "open",
        "type": "forecast"
    })
    if not data:
        return markets
...
def fetch_top_traders(limit=20):
    traders = []
    # Pull from the Data API leaderboard/holders endpoint
...
def find_arbitrage(all_markets, threshold=0.04):
    """
    Find markets across platforms with the same topic where
    combined implied probabilities suggest a gap > threshold.
    """
    arb_opportunities = []
    seen = []
