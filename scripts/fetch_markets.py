...
def norm(platform, title, prob_yes, volume, url, end_date=None, category=None, market_id=None):
    return {
        ...
        "prob_yes": round(float(prob_yes), 4), "prob_no": round(1 - float(prob_yes), 4),
        "volume": volume, "url": url, "end_date": end_date,
        "category": category or "general", "fetched_at": NOW,
    }

# ── Polymarket ──────────────────────────────────────────────────────────────────
def fetch_polymarket(limit=400):
    markets = []
    # Specifically target high-velocity tags
    tags = ["Sports", "Politics", "Crypto", "Fed"]
    for tag in tags:
        data = safe_get("https://gamma-api.polymarket.com/markets", params={
            "limit": 100, "active": "true", "closed": "false",
            "tag": tag, "_order": "volume24hr", "_sort": "desc"
        })
        ...
        for m in items:
            try:
                ...
                markets.append(norm(
                    platform="Polymarket", title=m.get("question", m.get("title", "")),
                    prob_yes=float(prices[yes_idx]), volume=m.get("volumeNum", 0),
                    url=f"https://polymarket.com/event/{m.get('slug', '')}",
                    end_date=m.get("endDate"), category=tag.lower(), market_id=m.get("id"),
                ))
            except: pass
...
def fetch_kalshi(limit=400):
    ...
    if data:
        for m in data.get("markets", []):
            try:
                markets.append(norm(
                    platform="Kalshi", title=m.get("title", ""),
                    prob_yes=m.get("yes_price", 50) / 100.0, volume=m.get("volume", 0),
                    url=f"https://kalshi.com/markets/{m.get('event_ticker','').lower()}",
                    end_date=m.get("close_time"), category=m.get("category", "general"),
    ...
    if data:
        for m in (data if isinstance(data, list) else []):
            try:
                ...
                markets.append(norm(
                    platform="Manifold", title=m.get("question", ""),
                    prob_yes=m.get("probability", 0.5), volume=m.get("volume", 0),
                    url=m.get("url", ""), end_date=m.get("closeTime"),
                    category="general", market_id=m.get("id"),
                ))
            except: pass
...
def fetch_metaculus(limit=400):
    ...
    data = safe_get("https://www.metaculus.com/api2/questions/", params={
        "limit": limit, "order_by": "-activity", "status": "open", "type": "forecast"
    })
    if data:
        for m in data.get("results", []):
            try:
                ...
                markets.append(norm(
                    platform="Metaculus", title=m.get("title", ""),
                    prob_yes=prob or 0.5, volume=m.get("number_of_predictions", 0),
                    url=f"https://www.metaculus.com{m.get('page_url', '')}",
                    end_date=m.get("resolve_time"), category="general", market_id=str(m.get("id")),
                ))
            except: pass
...
def find_arbitrage(all_markets, threshold=0.012):
    """
    Super-aggressive arb detector. Lowered threshold to 1.2% to catch 
    live sports gaps.
    """
    arb_opportunities = []
            ...
            if overlap >= 0.25:
                ...
                if diff >= threshold:
                    arb_opportunities.append({
                        ...
                        "prob_diff": round(diff, 4), "detected_at": NOW,
                    })
...
if __name__ == "__main__":
    ...
    with open("data/markets.json", "w") as f: json.dump({"updated_at": NOW, "markets": all_markets}, f, indent=2)
    with open("data/pro_bets.json", "w") as f: json.dump({"updated_at": NOW, "bets": pro_bets}, f, indent=2)
    with open("data/arbitrage.json", "w") as f: json.dump({"updated_at": NOW, "opportunities": arb}, f, indent=2)
    print(f"
Done. {len(all_markets)} markets. {len(arb)} arb ops found.")
